...
def embedFunc(text, model=embedding_model):             #embedding function
    text = text.replace("\n", " ")
    result = openai.Embedding.create(input = [text], model=model)      #This Works but only for like 2 values any more you need the paid version doesn't really tell you the rates. I will upgrade later and test
    embeddings = result['data'][0]['embedding']
    return embeddings
    
    
df['embedding'] = df['text'].apply(lambda x: embedFunc(x, embedding_model)) #making new coulum with embedded items
