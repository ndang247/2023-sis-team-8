...
def test_embedded_search_invalid_top_k():
    response = client.post(
        "/embedded_search",
        json={"text": "What courses are offered at UTS?", "top_k": "two"},
    )
    assert response.status_code == 422
...
def test_embedded_search_valid():
    response = client.post(
        "/embedded_search",
        json={"text": "What courses are offered at UTS?", "top_k": 1},
    )
    assert response.status_code == 200
...
def test_embedded_search_multiple_top_k():
    response = client.post(
        "/embedded_search",
        json={"text": "What engineering courses are offered at UTS?", "top_k": 3},
    )
    assert response.status_code == 200
...
def test_chat_read_existent_id():
    ...
    assert (
        ...
        == "Hello! What engineering courses does UTS offer?"
    )
...
def test_chat_read_non_existent_id():
    ...
    assert response.json()["detail"] == "'63080c44cf54ac1d6409a1be' does not exist!"
